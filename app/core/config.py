...
class Settings(BaseSettings):
    ...
    #
    # 비밀번호는 보낼 때 SHA-256 으로 바꿔 나간다 (다짐 방식). 여기에는 평문을 둔다.
    dajim_login_email: str = ""
    dajim_login_pw: str = ""

    #: 브로제이를 보는 지점 이름 — **그룹이 하나뿐이라** 이 지점만 브로제이를 탄다.
    #:
    #: 다짐 지점은 `branches.dajim_gym_id` 로 갈리므로 이 값과 무관하다.
    #: 여기 없는 지점이 출석 이력 주소를 열면 '연동이 아직 없습니다' 로 막힌다 —
    #: 안 막으면 **화순 명단이 다른 지점 이름표를 달고** 나간다.

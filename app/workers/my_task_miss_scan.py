...
## 얼마나 깎나 — **빠뜨린 개수만큼** (2026-09-18)

하루치 기본 -20 에, 항목이 하나 늘 때마다 -10 이 더 붙는다
(`1개 -20 · 3개 -40 · 9개 -100`). 며칠째인지는 안 본다 ([task_miss_points]).
...
#: 확정 누락 차감 — **그날 빠뜨린 항목 수로 매긴다** (2026-09-18 대표 결정).
#:
#: 하루치 기본이 [TASK_MISS_BASE] 이고, 항목이 하나 늘 때마다
#: [TASK_MISS_PER_TASK] 가 더 붙는다.
#:
#: ```
#: 1개 → -20    3개 → -40    5개 → -60    9개 → -100
#: ```
#:
#: **며칠째 누락인지는 안 본다.** 2026-09-16 에 지각처럼 쌓는 규칙
#: (`-10/-20/-30` + 5근무일 리셋)을 넣었다가 이틀 만에 걷어냈다 — 축이
#: 둘이 되면 같은 3개를 빠뜨려도 날마다 값이 달라져서 **직원이 자기 점수를
#: 미리 셀 수가 없다.** 지금은 빠뜨린 개수만 세면 된다.
#:
#: 빠뜨린 개수가 곧 그날 안 한 일의 양이라, 늘 빠뜨리는 사람은 개수로
#: 이미 무겁게 문다 — 쌓기를 없애도 "처음 빠뜨린 사람과 같은 값" 이 아니다.
TASK_MISS_BASE = -20
TASK_MISS_PER_TASK = -10


def task_miss_points(task_count: int) -> int:
    """그날 [task_count] 개를 빠뜨렸을 때 깎을 점수 (음수).

    항목이 0개면 누락 줄 자체가 안 생기지만, 혹시 0이 와도 기본값만 문다.
    """
    return TASK_MISS_BASE + TASK_MISS_PER_TASK * max(task_count - 1, 0)
...
async def my_task_miss_scan(now: datetime | None = None) -> None:
    ...
    async with SessionLocal() as db:
        ...
        for person, missed_on, contents in found:
            ...
            points = task_miss_points(len(contents))
            db.add(miss)
            await db.flush()
            event = await accrue_score(
                ...
                reason=f"{missed_on.month}월 {missed_on.day}일 개인 업무 누락 {len(contents)}건",
            )
            ...
            await notify(
                db,
                employee_id=person.id,
                **ntext.task_miss_confirmed(missed_on, contents, points),
            )
            made += 1

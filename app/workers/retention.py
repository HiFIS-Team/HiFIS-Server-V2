...
async def purge_old_access_logs() -> None:
    """보존기간(기본 90일)이 지난 접속 로그·활동 로그·응답 지표·이상행동을 파기한다.

    넷 다 같은 기간을 쓴다 — 성격이 같은 기록이라 한쪽만 오래 남길 이유가 없다.
    ...
    async with SessionLocal() as db:
        ...
        await db.commit()
        if access.rowcount or audit.rowcount or metric.rowcount or anomaly.rowcount:
            logger.info(
                "로그 파기: 접속 %d건 · 활동 %d건 · 지표 %d칸 · 이상행동 %d건 (%s 이전)",
                access.rowcount,
                audit.rowcount,
                metric.rowcount,
                anomaly.rowcount,

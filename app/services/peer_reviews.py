...
#:
#: 몇 명을 빠뜨렸든 한 번만 깎는다 — 사람 수로 곱하면 인원이 많은 지점이
#: 불리해진다.
#:
#: **여기는 고정이다.** 개인 업무 누락은 2026-09-16 에 누적(`TASK_MISS_PENALTY`)
#: 으로 갈았는데, 저쪽은 매일 걸리는 일이라 쌓을 자리가 있고 이쪽은 **한 달에
#: 한 번**이라 쌓아도 여섯 달에 세 칸이다 — 그 사이에 잊는다.
PEER_MISS_POINTS = -20

...
class Branch(UUIDMixin, TimestampMixin, Base):
    ...
    #:
    #: 같은 값끼리만 서로 본다 — 지금은 `첨단`·`화순` 이 `A` 하나뿐이다.
    #:
    #: 원래는 `동광주` 를 `B` 로 갈라 두었는데(거기서 앱 화면이 밖으로 나간
    #: 일이 있었다), **2026-09-09 에 지점째 걷어내면서** 묶음이 하나만 남았다
    #: (`dgj000000001`). 장치는 그대로 둔다 — 다시 갈라야 할 때 값만 주면 된다.
    #:

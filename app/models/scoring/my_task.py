...
class MyTask(UUIDMixin, TimestampMixin, Base):
    ...
    #: **달 며칠에 도는가** — `[1, 15]` 면 매달 1일과 15일 (2026-09-21 요청).
    #:
    #: 비어 있으면(`None`) 요일로 도는 업무다 — [weekdays] 가 정한다.
    #: 값이 있으면 **[weekdays] 를 안 본다.** 둘을 같이 걸면 '매달 1일이면서
    #: 금요일' 같은 뜻이 되는데, 만드는 화면이 주냐 월이냐 하나를 고르게
    #: 돼 있어서 그런 업무를 만들 길이 없다.
    #:
    #: **요일 칸을 날짜로 바꿔 쓰지 않는다.** 1~7 이 월요일인지 1일인지를
    #: 값만 보고 못 가르고, 이미 쌓인 업무는 전부 `{1..7}`(매일) 이라
    #: 그대로 날짜로 읽히면 매달 1~7일에만 서게 된다.
    #:
    #: 없는 날은 그 달에 안 선다 — 31일짜리 업무는 2월에 안 돈다.
    #: 말일로 당기지 않는다. 당기면 2월 28일에 서는데, 그날 제 차례인 다른
    #: 업무와 섞여서 왜 오늘 섰는지가 화면에서 안 보인다.
    monthdays: Mapped[list[int] | None] = mapped_column(

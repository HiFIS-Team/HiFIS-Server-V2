...
class InboxItemOut(CamelModel):
    ...
    detail: str              # 한 줄 요약
    #: 올린 사람이 **적어 낸 글** — 지금은 업무 누락 사유서만 채운다 (2026-09-09).
    #:
    #: [detail] 과 자리가 다르다 — 저기는 서버가 만든 한 줄 요약(`9월 5일 · 3개`)
    #: 이고, 여기는 **사람이 쓴 문장**이라 화면이 여러 줄로 편다.
    #:
    #: 없으면 안 그린다. 결재하는 쪽이 이걸 못 읽으면 승인·반려를 판단할 근거가
    #: 없다 — 사유서를 받아 놓고 아무도 안 읽는 상태였다 (대표 지적).
    reason: str | None = None
    #: **매장 TV 에 이렇게 걸린다** — 승인하면 벽에 나갈 문장 (2026-09-09).
    #:
    #: 컴플레인·클레임해결만 채워진다. 신청할 때 한 번 다듬어 박아 둔 값이라
    #: 여기서 만들지 않는다 — 결재함을 열 때마다 만들면 볼 때마다 달라진다.
    #:
    #: 결재하는 쪽이 **누르기 전에** 보고 판단하라고 싣는다. 예전에는 승인
    #: 뒤에 만들어져서, 이상하게 다듬어져도 지우는 것 말고는 손쓸 수 없었다.
